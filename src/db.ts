...
export type Recurrence = 'none' | 'monthly' | 'quarterly' | 'yearly' | 'every_3_years'
...
export interface Utility {
  ...
  type: 'electric_siirto' | 'electric_energia' | 'water' | 'gas' | 'internet' | 'waste'
  amount: number
...
export interface HeatingSystem {
  ...
  type: 'wood' | 'oil' | 'geothermal' | 'air_heat_pump' | 'electric' | 'district'
...
export type FireplaceType =
  | 'bakery_oven' | 'fireplace' | 'sauna_stove' | 'masonry_heater' | 'chimney'
  | 'kamina' | 'water_boiler' | 'wood_stove'
...
export interface WastewaterSystem {
  ...
  type: 'septic_tank' | 'sealed_tank' | 'soil_filter' | 'small_treatment' | 'mains_sewer'
...
export type DocumentLinkType = '' | 'fireplace' | 'wastewater' | 'water_test' | 'insurance'
...
const migrations: ((db: any) => void)[] = [
  // v1: lisää billing_month + provider sarakkeet vanhempiin utilities-tauluihin
  (db) => {
    const cols = (db.prepare("PRAGMA table_info(utilities)").all() as { name: string }[]).map(c => c.name)
    if (!cols.includes('billing_month')) {
      db.exec("ALTER TABLE utilities ADD COLUMN billing_month TEXT NOT NULL DEFAULT ''")
    }
    if (!cols.includes('provider')) {
      db.exec("ALTER TABLE utilities ADD COLUMN provider TEXT NOT NULL DEFAULT ''")
    }
  },
  ...
  (db) => {
    ...
    if (!cols.includes('sauna_type')) {
      db.exec("ALTER TABLE properties ADD COLUMN sauna_type TEXT NOT NULL DEFAULT 'none'")
    }
    if (!cols.includes('sauna_info')) {
      db.exec("ALTER TABLE properties ADD COLUMN sauna_info TEXT NOT NULL DEFAULT ''")
    }
    if (!cols.includes('property_tax')) {
      db.exec("ALTER TABLE properties ADD COLUMN property_tax REAL NOT NULL DEFAULT 0")
    }
    if (!cols.includes('road_fee')) {
      db.exec("ALTER TABLE properties ADD COLUMN road_fee REAL NOT NULL DEFAULT 0")
    }
  },
  // v4: Vaihe 6 — jätevesijärjestelmän vaatimustenmukaisuuden arvioinnin kentät.
  (db) => {
    const cols = (db.prepare("PRAGMA table_info(wastewater_systems)").all() as { name: string }[]).map(c => c.name)
    const add = (name: string, decl: string) => { if (!cols.includes(name)) db.exec(`ALTER TABLE wastewater_systems ADD COLUMN ${name} ${decl}`) }
  ...
  (db) => {
    const cols = (db.prepare("PRAGMA table_info(properties)").all() as { name: string }[]).map(c => c.name)
    const add = (name: string, decl: string) => { if (!cols.includes(name)) db.exec(`ALTER TABLE properties ADD COLUMN ${name} ${decl}`) }
  ...
  (db) => {
    ...
    if (!cols.includes('linked_type')) db.exec("ALTER TABLE documents ADD COLUMN linked_type TEXT NOT NULL DEFAULT ''")
    if (!cols.includes('linked_id')) db.exec("ALTER TABLE documents ADD COLUMN linked_id INTEGER NOT NULL DEFAULT 0")
  },
  ...
  (db) => {
    ...
    `)
    db.exec('INSERT INTO fireplaces (id, property_id, type, name, last_sweep, next_sweep, sweeper) SELECT id, property_id, type, name, last_sweep, next_sweep, sweeper FROM fireplaces_old')
...
function seedData(db: DatabaseSync) {
  ...
  insertProperty.run("Metsäpirtti", "405-412-1-23", "well", 1948, "Sysmä, Finland", "wood", "Erillinen puukiuassauna pihapiirissä", 168.0, 120.0,
    "3×25 A", "Oma kaivo (rengaskaivo)", "Kiertokaari", "240 l", "4 vk", "home_compost", 0, "")
  insertProperty.run("Järvenranta", "405-412-1-24", "well", 1965, "Sysmä, Finland", "wood", "Rantasauna, Harvia puukiuas", 142.0, 120.0,
    "3×25 A", "Oma kaivo (porakaivo)", "Kiertokaari", "140 l", "8 vk", "shared", 0, "")
  insertProperty.run("Pappila", "837-112-2-45", "mains", 1910, "Tampere, Finland", "electric", "Talon sauna, sähkökiuas", 410.0, 0.0,
    "3×35 A", "DN32 (kunnan vesi)", "Kiertokaari", "660 l", "2 vk", "collection", 0, "")

  // Fetch created property IDs
  const props = db.prepare("SELECT id, name FROM properties").all() as { id: number; name: string }[]
  ...
  insertTask.run(metsaId, "Kaivoveden laadun analyysi", "pending", "high", "2026-06-15", "Vesi", 120, "every_3_years", "2029-06-15")
  insertTask.run(metsaId, "Hormien nuohous", "completed", "high", "2026-05-10", "Paloturvallisuus", 65, "yearly", "2027-05-10")
  insertTask.run(metsaId, "Klapien pinoaminen talveksi", "in_progress", "low", "2026-08-30", "Piha", 0, "yearly", "2027-08-30")

  // Järvenranta (Well water, lakeside cottage)
  insertTask.run(jarviId, "Saunan ulkoseinien tervaus", "pending", "medium", "2026-07-20", "Ylläpito", 150, "none", null)
  insertTask.run(jarviId, "Kaivopumpun suodattimen vaihto", "completed", "high", "2026-05-25", "Vesi", 30, "yearly", "2027-05-25")

  // Pappila (Mains water, old rectory log building in town)
  insertTask.run(pappilaId, "Rossipohjan luukkujen korjaus", "in_progress", "high", "2026-06-20", "Rakenne", 200, "none", null)
  insertTask.run(pappilaId, "Leivinuunin hormin nuohous", "pending", "medium", "2026-09-01", "Paloturvallisuus", 85, "yearly", "2027-09-01")
  ...
  const insertRenovation = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `)
  insertRenovation.run(pappilaId, "Hirsikertojen korjaustyöt", "in_progress", 12000, 9400, "2026-05-01", null)
  insertRenovation.run(jarviId, "Kiuasremontti ja piippu", "completed", 1200, 1150, "2026-04-10", "2026-05-03")
  insertRenovation.run(metsaId, "Aurinkosähköjärjestelmä", "planning", 3000, 0, "2026-07-15", null)
  ...
  `)
  // Income
  insertTransaction.run(metsaId, "income", "Vuokraus", 650.00, "2026-05-28", "Mökin vuokratulo vkl")
  insertTransaction.run(pappilaId, "income", "Vuokraus", 1200.00, "2026-05-01", "Kuukausivuokra")
  insertTransaction.run(pappilaId, "income", "Metsätalous", 350.00, "2026-04-15", "Polttopuiden myynti")

  // Expenses
  insertTransaction.run(pappilaId, "expense", "Remontti", 9400.00, "2026-05-12", "Hirsikorjauksen ennakko")
  insertTransaction.run(jarviId, "expense", "Remontti", 1150.00, "2026-04-20", "Harvia puukiuas ja teräspiippu")
  insertTransaction.run(metsaId, "expense", "Vesi", 120.00, "2026-06-01", "Kaivoveden laboratoriotutkimus")
  insertTransaction.run(jarviId, "expense", "Kalusto", 145.00, "2026-05-05", "Fiskars X27 halkaisukirves")
  ...
  insertUtility.run(pappilaId, "electric_siirto", 98.40, "2026-05-20", "2026-05", 840, "Caruna")
  insertUtility.run(pappilaId, "electric_energia", 112.00, "2026-05-20", "2026-05", 840, "Fortum")
  insertUtility.run(pappilaId, "water", 48.60, "2026-05-15", "2026-05", 14, "Kunnan vesilaitos")
  insertUtility.run(pappilaId, "waste", 24.50, "2026-05-10", "2026-05", 1, "Kiertokaari")
  // Pappila huhtikuu
  insertUtility.run(pappilaId, "electric_siirto", 105.20, "2026-04-20", "2026-04", 920, "Caruna")
  insertUtility.run(pappilaId, "electric_energia", 124.80, "2026-04-20", "2026-04", 920, "Fortum")

  // Metsäpirtti (kaivovesi, sähkö mökkiin)
  insertUtility.run(metsaId, "electric_siirto", 32.10, "2026-05-22", "2026-05", 210, "Caruna")
  insertUtility.run(metsaId, "electric_energia", 42.10, "2026-05-22", "2026-05", 210, "Helen")
  insertUtility.run(metsaId, "waste", 18.00, "2026-05-08", "2026-05", 1, "Kiertokaari")

  // Järvenranta (kaivovesi)
  insertUtility.run(jarviId, "electric_siirto", 15.20, "2026-05-25", "2026-05", 85, "Caruna")
  insertUtility.run(jarviId, "electric_energia", 20.60, "2026-05-25", "2026-05", 85, "Helen")
  ...
  const insertTool = db.prepare(`
    ...
    VALUES (?, ?, ?, ?)
  `)
  insertTool.run("Fiskars halkaisukirves X27", "working", "Järvenrannan liiteri", "2026-05-05")
  insertTool.run("Husqvarna 130 Moottorisaha", "working", "Pappilan autotalli", "2025-09-12")
  insertTool.run("Kaivopumpun painekytkin", "needs_repair", "Metsäpirtin kellari", "2024-06-15")
  insertTool.run("Kuorimarauta hirsille", "working", "Metsäpirtin liiteri", "2026-04-10")
  ...
  const insertInsurance = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `)
  insertInsurance.run(metsaId, "Mökin täysarvovakuutus", "LähiTapiola", 340.00, "2026-11-01", "Hirsirakennus, irtaimisto ja palo")
  insertInsurance.run(jarviId, "Rantasaunan perusvakuutus", "Pohjola", 180.00, "2026-12-10", "Saunarakennus, palo- ja luonnonilmiöt")
  insertInsurance.run(pappilaId, "Päärakennuksen suojeluvakuutus", "If vakuutus", 850.00, "2027-02-15", "Kulttuurihistoriallinen puutalo")
  ...
  const insertHeating = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `)
  insertHeating.run(metsaId, "wood", "Puukattila + 2000 l varaaja, tukena ilmalämpöpumppu", null, null)
  insertHeating.run(jarviId, "electric", "Suora sähkölämmitys + puukiuas saunassa", null, null)
  // Öljysäiliö vaatii määräaikaistarkastuksen (esim. 10 v välein)
  insertHeating.run(pappilaId, "oil", "1500 l öljysäiliö kellarissa, öljykattila", "2019-08-01", "2029-08-01")
  ...
  const insertFireplace = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `)
  insertFireplace.run(metsaId, "masonry_heater", "Olohuoneen varaava takka", "2026-05-10", "2027-05-10", "Sysmän nuohouspalvelu")
  insertFireplace.run(metsaId, "sauna_stove", "Saunan puukiuas", "2026-05-10", "2027-05-10", "Sysmän nuohouspalvelu")
  insertFireplace.run(jarviId, "sauna_stove", "Rantasaunan puukiuas", "2026-04-20", "2027-04-20", "Sysmän nuohouspalvelu")
  insertFireplace.run(pappilaId, "bakery_oven", "Keittiön leivinuuni", "2025-09-01", "2026-09-01", "Tampereen Nuohous Oy")
  insertFireplace.run(pappilaId, "fireplace", "Salin kaakeliuuni", "2025-09-01", "2026-09-01", "Tampereen Nuohous Oy")
  ...
  insertWastewater.run(metsaId, "septic_tank", "Saostuskaivo + kivipesäimeytys (1973). Ei ranta- eikä pohjavesialuetta.", "2026-05-01", "2027-05-01", "Lakeuden Loka", 1973, 0, 0, 1, 0)
  // Järvenranta: rantakohde, mutta umpisäiliö (hyväksytty ratkaisu herkälläkin alueella)
  insertWastewater.run(jarviId, "sealed_tank", "Umpisäiliö 5 m³ (WC), harmaavesille erillinen imeytys", "2026-06-01", "2026-09-01", "Lakeuden Loka", 2016, 1, 0, 1, 0)
  insertWastewater.run(pappilaId, "mains_sewer", "Liitetty kunnalliseen viemäriverkkoon", null, null, "", 2010, 0, 0, 1, 0)
  ...
  const insertWaterTest = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `)
  insertWaterTest.run(metsaId, "2023-06-14", "0 pmy/100ml", "0 pmy/100ml", "2.1 mg/l", "6.8", "0.05 mg/l", "0.3 mg/l", 1, "Täyttää talousvesivaatimukset (STM 1352/2015)")
  insertWaterTest.run(jarviId, "2024-07-02", "0 pmy/100ml", "3 pmy/100ml", "1.4 mg/l", "6.2", "0.4 mg/l", "0.2 mg/l", 0, "Lievä rautapitoisuus ja koliformit — suositellaan uusintanäytettä")
  ...
  const insertFirewood = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `)
  insertFirewood.run(metsaId, "Koivu", 8.0, "pino-m³", "Metsäpirtin klapiliiteri", "ready", "2025-05-20", "Kuivunut kesän yli, käyttövalmis talveksi")
  insertFirewood.run(metsaId, "Sekapuu", 4.0, "pino-m³", "Metsäpirtin liiteri", "drying", "2026-06-01", "Kaadettu keväällä, kuivumassa")
  insertFirewood.run(jarviId, "Leppä", 2.5, "pino-m³", "Rantasaunan puuvaja", "ready", "2025-08-10", "Saunapuut")
  insertFirewood.run(pappilaId, "Koivu", 6.0, "motti", "Pappilan autotalli", "ready", "2025-09-15", "Ostoklapit, kuivat")
  ...
  const insertBooking = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `)
  insertBooking.run(metsaId, "Virtanen", "2026-06-19", "2026-06-22", 285.0, "completed", 1, "Juhannusviikonloppu")
  insertBooking.run(metsaId, "Korhonen", "2026-07-27", "2026-08-03", 560.0, "confirmed", 0, "Viikon vuokraus, koko perhe")
  insertBooking.run(jarviId, "Nieminen", "2026-08-14", "2026-08-16", 190.0, "tentative", 0, "Alustava varaus, odottaa vahvistusta")
  insertBooking.run(pappilaId, "Mäkelä", "2026-07-04", "2026-07-06", 240.0, "confirmed", 0, "Häävieraat")
  ...
  const insertContact = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `)
  insertContact.run("Sysmän nuohouspalvelu", "nuohooja", "040 123 4567", "info@sysmannuohous.fi", "Piirinuohooja, Sysmän alue")
  insertContact.run("Tampereen Nuohous Oy", "nuohooja", "03 234 5678", "asiakas@trenuohous.fi", "Pappilan hormit")
  insertContact.run("Lakeuden Loka", "loka", "0200 12345", "tilaus@lakeudenloka.fi", "Sakokaivojen ja umpisäiliöiden tyhjennys")
  insertContact.run("LVI-Virtanen", "lvi", "045 987 6543", "", "Vesipumput ja putkistot")
  insertContact.run("Sähkö-Mäkinen", "sahko", "050 555 1212", "", "Sähkötyöt ja tarkastukset")
  ...
  const insertDocument = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `)
  insertDocument.run(metsaId, "deed", "Lainhuutotodistus", "~/Documents/metsapirtti-lainhuuto.pdf", "2018-03-12", "Lainhuuto rekisteröity", "", 0)
  insertDocument.run(metsaId, "inspection", "Kaivoveden tutkimustodistus 2023", "~/Documents/metsapirtti-vesi-2023.pdf", "2023-06-20", "Laboratorion lausunto", "", 0)
  insertDocument.run(pappilaId, "permit", "Rakennuslupa hirsikorjaus", "~/Documents/pappila-rakennuslupa.pdf", "2026-04-28", "Tampereen rakennusvalvonta", "", 0)
  insertDocument.run(pappilaId, "purchase", "Kauppakirja", "~/Documents/pappila-kauppakirja.pdf", "2010-09-01", "", "", 0)
  insertDocument.run(jarviId, "warranty", "Harvia-kiukaan takuutodistus", "~/Documents/jarvenranta-kiuas-takuu.pdf", "2026-05-03", "Takuu 2 vuotta", "", 0)

  // Nuohoustodistus liitettynä Metsäpirtin varaavaan takkaan (ensimmäinen fireplace-rivi)
  const firstMetsaFireplace = db.prepare("SELECT id FROM fireplaces WHERE property_id = ? ORDER BY id ASC LIMIT 1").get(metsaId) as { id: number } | undefined
  if (firstMetsaFireplace) {
    insertDocument.run(metsaId, "inspection", "Nuohoustodistus 2026", "~/Documents/metsapirtti-nuohous-2026.pdf", "2026-05-10", "Sysmän nuohouspalvelu", "fireplace", firstMetsaFireplace.id)
  }
  ...
  const insertMeter = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `)
  insertMeter.run(pappilaId, "electric", 24500, "2026-04-01", "Vuosineljänneksen alku")
...
export function getTasks(propertyId?: number): Task[] {
  ...
  const query = propertyId 
    ? db.prepare("SELECT * FROM tasks WHERE property_id = ? ORDER BY status ASC, due_date ASC")
...
export function getRenovations(propertyId?: number): Renovation[] {
  const db = initDb()
  const query = propertyId 
    ? db.prepare("SELECT * FROM renovations WHERE property_id = ? ORDER BY status DESC, start_date DESC")
    : db.prepare("SELECT * FROM renovations ORDER BY status DESC, start_date DESC")
...
export function getTransactions(propertyId?: number): Transaction[] {
  const db = initDb()
  const query = propertyId 
    ? db.prepare("SELECT * FROM transactions WHERE property_id = ? ORDER BY date DESC")
...
export function getUtilities(propertyId?: number): Utility[] {
  const db = initDb()
  const query = propertyId
    ? db.prepare("SELECT * FROM utilities WHERE property_id = ? ORDER BY billing_month DESC, type ASC")
    : db.prepare("SELECT * FROM utilities ORDER BY billing_month DESC, property_id ASC, type ASC")
...
export function getInsurance(propertyId?: number): Insurance[] {
  const db = initDb()
  const query = propertyId
    ? db.prepare("SELECT * FROM insurance WHERE property_id = ? ORDER BY renewal_date ASC")
...
export function addTask(task: Omit<Task, 'id'>): void {
  ...
  `)
  stmt.run(
    task.property_id, task.title, task.status, task.priority, task.due_date,
    task.category, task.cost, task.recurrence ?? 'none', task.next_due ?? null
  )
}
...
export function addTransaction(tx: Omit<Transaction, 'id'>): void {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `)
  stmt.run(tx.property_id, tx.type, tx.category, tx.amount, tx.date, tx.description)
}
...
export function advanceRecurrence(dateStr: string, recurrence: Recurrence): string {
  if (recurrence === 'none') return dateStr
  const months = recurrence === 'monthly' ? 1
...
export function updateTaskStatus(id: number, status: Task['status']): void {
  ...
  `)
  insertNext.run(
    task.property_id, task.title, task.priority, newDueDate,
    task.category, task.cost, task.recurrence, newNextDue
  )
}
...
export function updateTask(task: Task): void {
  ...
  `)
  stmt.run(
    task.property_id, task.title, task.priority, task.category, task.cost,
    task.recurrence ?? 'none', task.next_due ?? null, task.id
  )
}
...
export function updateTransaction(tx: Transaction): void {
  ...
  const stmt = db.prepare(`
    ...
    WHERE id = ?
  `)
  stmt.run(tx.property_id, tx.type, tx.category, tx.amount, tx.description, tx.id)
}
...
export function addUtility(u: Omit<Utility, 'id'>): void {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `)
  stmt.run(u.property_id, u.type, u.amount, u.billing_date, u.billing_month, u.usage_value, u.provider)
}
...
export function updateUtility(u: Utility): void {
  ...
  const stmt = db.prepare(`
    ...
    WHERE id = ?
  `)
  stmt.run(u.property_id, u.type, u.amount, u.billing_date, u.billing_month, u.usage_value, u.provider, u.id)
}

const PROP_EXTRA_COLS = 'sauna_type, sauna_info, property_tax, road_fee, electricity_fuse, water_connection, waste_provider, waste_bin, waste_interval, biowaste, compost_registered, compost_reg_date'
function propExtraValues(prop: Omit<Property, 'id'>): unknown[] {
  return [
    prop.sauna_type ?? 'none', prop.sauna_info ?? '', prop.property_tax ?? 0, prop.road_fee ?? 0,
    prop.electricity_fuse ?? '', prop.water_connection ?? '', prop.waste_provider ?? '', prop.waste_bin ?? '',
    prop.waste_interval ?? '', prop.biowaste ?? 'collection', prop.compost_registered ?? 0, prop.compost_reg_date ?? ''
  ]
}
...
export function addProperty(prop: Omit<Property, 'id'>): void {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ${PROP_EXTRA_COLS.split(',').map(() => '?').join(', ')})
  `)
  stmt.run(prop.name, prop.kiinteistotunnus, prop.water_source, prop.build_year, prop.location, ...propExtraValues(prop))
}

export function updateProperty(prop: Property): void {
  const db = initDb()
  const setClause = ['name', 'kiinteistotunnus', 'water_source', 'build_year', 'location', ...PROP_EXTRA_COLS.split(',').map(c => c.trim())]
    .map(c => `${c} = ?`).join(', ')
  const stmt = db.prepare(`UPDATE properties SET ${setClause} WHERE id = ?`)
  stmt.run(prop.name, prop.kiinteistotunnus, prop.water_source, prop.build_year, prop.location, ...propExtraValues(prop), prop.id)
}
...
export function assessComposting(p: Property): CompostAssessment | null {
  ...
  if (!p.compost_registered) {
    return { level: 'warning', message: 'Kotikompostointi on ilmoitettava kunnan jätehuoltoviranomaiselle — ilmoitus puuttuu.' }
  }
  return { level: 'ok', message: `Kompostointi-ilmoitus tehty${p.compost_reg_date ? ` (${p.compost_reg_date})` : ''}.` }
}
...
export function addRenovation(ren: Omit<Renovation, 'id'>): void {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `)
  stmt.run(ren.property_id, ren.project_name, ren.status, ren.budget, ren.spent, ren.start_date, ren.end_date)
}
...
export function updateRenovation(ren: Renovation): void {
  ...
  const stmt = db.prepare(`
    ...
    WHERE id = ?
  `)
  stmt.run(ren.property_id, ren.project_name, ren.status, ren.budget, ren.spent, ren.start_date, ren.end_date, ren.id)
}
...
export function addInsurance(i: Omit<Insurance, 'id'>): void {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `)
  stmt.run(i.property_id, i.policy_name, i.provider, i.premium, i.renewal_date, i.coverage_details)
}
...
export function updateInsurance(i: Insurance): void {
  ...
  const stmt = db.prepare(`
    ...
    WHERE id = ?
  `)
  stmt.run(i.property_id, i.policy_name, i.provider, i.premium, i.renewal_date, i.coverage_details, i.id)
}
...
export function getHeatingSystems(propertyId?: number): HeatingSystem[] {
  const db = initDb()
  const query = propertyId
    ? db.prepare("SELECT * FROM heating_systems WHERE property_id = ? ORDER BY next_inspection ASC")
    : db.prepare("SELECT * FROM heating_systems ORDER BY property_id ASC, next_inspection ASC")
...
export function addHeatingSystem(h: Omit<HeatingSystem, 'id'>): void {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `)
  stmt.run(h.property_id, h.type, h.description, h.last_inspection ?? null, h.next_inspection ?? null)
}
...
export function updateHeatingSystem(h: HeatingSystem): void {
  ...
  const stmt = db.prepare(`
    ...
    WHERE id = ?
  `)
  stmt.run(h.property_id, h.type, h.description, h.last_inspection ?? null, h.next_inspection ?? null, h.id)
}

export function getFireplaces(propertyId?: number): Fireplace[] {
  const db = initDb()
  const query = propertyId
    ? db.prepare("SELECT * FROM fireplaces WHERE property_id = ? ORDER BY next_sweep ASC")
    : db.prepare("SELECT * FROM fireplaces ORDER BY property_id ASC, next_sweep ASC")
...
export function addFireplace(f: Omit<Fireplace, 'id'>): void {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `)
  stmt.run(f.property_id, f.type, f.name, f.last_sweep ?? null, f.next_sweep ?? null, f.sweeper)
}
...
export function updateFireplace(f: Fireplace): void {
  ...
  const stmt = db.prepare(`
    ...
    WHERE id = ?
  `)
  stmt.run(f.property_id, f.type, f.name, f.last_sweep ?? null, f.next_sweep ?? null, f.sweeper, f.id)
}

export function getWastewaterSystems(propertyId?: number): WastewaterSystem[] {
  const db = initDb()
  const query = propertyId
    ? db.prepare("SELECT * FROM wastewater_systems WHERE property_id = ? ORDER BY next_emptied ASC")
    : db.prepare("SELECT * FROM wastewater_systems ORDER BY property_id ASC, next_emptied ASC")
  return (propertyId ? query.all(propertyId) : query.all()) as WastewaterSystem[]
}
...
export function addWastewaterSystem(w: Omit<WastewaterSystem, 'id'>): void {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `)
  stmt.run(w.property_id, w.type, w.permit_info, w.last_emptied ?? null, w.next_emptied ?? null, w.emptying_provider,
    w.build_year ?? 0, w.shoreline ?? 0, w.groundwater ?? 0, w.has_wc ?? 1, w.exemption ?? 0)
}
...
export function updateWastewaterSystem(w: WastewaterSystem): void {
  ...
  `)
  stmt.run(w.property_id, w.type, w.permit_info, w.last_emptied ?? null, w.next_emptied ?? null, w.emptying_provider,
    w.build_year ?? 0, w.shoreline ?? 0, w.groundwater ?? 0, w.has_wc ?? 1, w.exemption ?? 0, w.id)
}
...
export function assessWastewater(w: WastewaterSystem): WastewaterAssessment {
  ...
  if (w.type === 'sealed_tank') {
    actions.push('Seuraa umpisäiliön täyttymistä ja tilaa tyhjennys ajoissa (täyttöhälytin suositeltava).')
  }

  // Kunnan viemäri: ei kiinteistökohtaisia puhdistusvaatimuksia.
  if (w.type === 'mains_sewer') {
    return { level: 'ok', headline: 'Liitetty kunnalliseen viemäriin — ei kiinteistökohtaisia puhdistusvaatimuksia.', issues: [], actions: [] }
  }
  ...
  if (w.type === 'septic_tank') {
    inadequate = true
    issues.push('Pelkkä saostuskaivo (esim. + kivipesäimeytys) on vain esikäsittely eikä täytä VNa 157/2017 puhdistusvaatimuksia (orgaaninen aine 80 % / fosfori 70 % / typpi 30 %).')
  } else if (w.type === 'soil_filter' && w.build_year > 0 && w.build_year < 2004) {
    inadequate = true
    issues.push('Ennen nykyvaatimuksia rakennettu maasuodattamo ei välttämättä täytä puhdistusvaatimuksia — tarkistuta kunto ja puhdistusteho.')
  }

  // Riittävä ratkaisu (umpisäiliö, pienpuhdistamo, uudehko maasuodattamo).
  if (!inadequate) {
    const hl = w.type === 'sealed_tank' ? 'Umpisäiliö on hyväksytty ratkaisu — huolehdi tyhjennyksestä.'
      : w.type === 'small_treatment' ? 'Pienpuhdistamo — huolehdi säännöllisestä huollosta ja seurannasta.'
  ...
  if (!w.has_wc) {
    issues.push('Ei vesikäymälää → "vähäiset jätevedet": vaatimukset kevyemmät (harmaavesien käsittely riittää usein).')
    return { level: 'warning', headline: 'Vähäiset jätevedet — kevyemmät vaatimukset, mutta varmista riittävyys.', issues, actions }
  }

  // Vapautus voimassa (ikä ≥ syntynyt ennen 9.3.1943 / vähäisyys).
  if (w.exemption) {
    issues.push('Vapautus merkitty voimassa olevaksi (ikä-/vähäisyysperuste) — voimassaolo kannattaa varmistaa kunnalta.')
    return { level: 'warning', headline: 'Puute tunnistettu, mutta vapautus voi olla voimassa — varmista kunnalta.', issues, actions }
  }

  // Herkkä alue (ranta ≤100 m tai pohjavesialue): lakisääteinen takaraja oli 31.10.2019.
  if (w.shoreline || w.groundwater) {
    const alue = w.shoreline && w.groundwater ? 'ranta- ja pohjavesialue' : w.shoreline ? 'ranta-alue (≤100 m vesistöstä)' : 'pohjavesialue'
    actions.unshift(`Herkkä alue (${alue}): saata järjestelmä vaatimusten mukaiseksi — lakisääteinen takaraja oli jo 31.10.2019.`)
    return { level: 'action', headline: `Toimenpide tarpeen: herkän alueen takaraja (31.10.2019) on ohitettu.`, issues, actions }
  }

  // Ei herkällä alueella: ei kiinteää takarajaa, korjaus kytkeytyy remonttiin.
  actions.unshift('Ei kiinteää takarajaa (ei herkkä alue): korjausvelvoite laukeaa luvanvaraisen remontin tai vesi-/viemäri-/wc-korjauksen yhteydessä. Suositus: suunnittele päivitys.')
  return { level: 'warning', headline: 'Puute tunnistettu — ei kiinteää takarajaa, mutta korjaus tulee tehdä remontin yhteydessä.', issues, actions }
}

export function getWaterTests(propertyId?: number): WaterTest[] {
  const db = initDb()
  const query = propertyId
    ? db.prepare("SELECT * FROM water_tests WHERE property_id = ? ORDER BY test_date DESC")
...
export function addWaterTest(t: Omit<WaterTest, 'id'>): void {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `)
  stmt.run(t.property_id, t.test_date, t.ecoli, t.coliforms, t.nitrate, t.ph, t.iron, t.fluoride, t.passed, t.notes)
}
...
export function updateWaterTest(t: WaterTest): void {
  ...
  const stmt = db.prepare(`
    ...
    WHERE id = ?
  `)
  stmt.run(t.property_id, t.test_date, t.ecoli, t.coliforms, t.nitrate, t.ph, t.iron, t.fluoride, t.passed, t.notes, t.id)
}
...
export function getFirewood(propertyId?: number): Firewood[] {
  const db = initDb()
  const query = propertyId
    ? db.prepare("SELECT * FROM firewood WHERE property_id = ? ORDER BY drying_status ASC, wood_type ASC")
    : db.prepare("SELECT * FROM firewood ORDER BY property_id ASC, drying_status ASC, wood_type ASC")
...
export function addFirewood(f: Omit<Firewood, 'id'>): void {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `)
  stmt.run(f.property_id, f.wood_type, f.volume, f.unit, f.location, f.drying_status, f.stacked_date, f.notes)
}
...
export function updateFirewood(f: Firewood): void {
  ...
  const stmt = db.prepare(`
    ...
    WHERE id = ?
  `)
  stmt.run(f.property_id, f.wood_type, f.volume, f.unit, f.location, f.drying_status, f.stacked_date, f.notes, f.id)
}
...
export function getBookings(propertyId?: number): Booking[] {
  const db = initDb()
  const query = propertyId
    ? db.prepare("SELECT * FROM bookings WHERE property_id = ? ORDER BY start_date ASC")
...
export function addBooking(b: Omit<Booking, 'id'>): void {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `)
  stmt.run(b.property_id, b.guest_name, b.start_date, b.end_date, b.price, b.status, b.income_recorded, b.notes)
}
...
export function updateBooking(b: Booking): void {
  ...
  const stmt = db.prepare(`
    ...
    WHERE id = ?
  `)
  stmt.run(b.property_id, b.guest_name, b.start_date, b.end_date, b.price, b.status, b.income_recorded, b.notes, b.id)
}

// Laskee öiden lukumäärän varauksessa (lähtö − saapuminen).
export function bookingNights(b: Pick<Booking, 'start_date' | 'end_date'>): number {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(b.start_date) || !/^\d{4}-\d{2}-\d{2}$/.test(b.end_date)) return 0
  const ms = new Date(`${b.end_date}T00:00:00Z`).getTime() - new Date(`${b.start_date}T00:00:00Z`).getTime()
...
export function getContacts(): Contact[] {
  ...
  return db.prepare("SELECT * FROM contacts ORDER BY role ASC, name ASC").all() as Contact[]
}

export function addContact(c: Omit<Contact, 'id'>): void {
  const db = initDb()
  const stmt = db.prepare("INSERT INTO contacts (name, role, phone, email, notes) VALUES (?, ?, ?, ?, ?)")
  stmt.run(c.name, c.role, c.phone, c.email, c.notes)
}

export function updateContact(c: Contact): void {
  const db = initDb()
  const stmt = db.prepare("UPDATE contacts SET name = ?, role = ?, phone = ?, email = ?, notes = ? WHERE id = ?")
...
export function getDocuments(propertyId?: number): Document[] {
  const db = initDb()
  const query = propertyId
    ? db.prepare("SELECT * FROM documents WHERE property_id = ? ORDER BY issued_date DESC")
    : db.prepare("SELECT * FROM documents ORDER BY property_id ASC, issued_date DESC")
...
export function addDocument(d: Omit<Document, 'id'>): void {
  ...
  const stmt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `)
  stmt.run(d.property_id, d.doc_type, d.title, d.file_path, d.issued_date, d.notes, d.linked_type ?? '', d.linked_id ?? 0)
}
...
export function updateDocument(d: Document): void {
  ...
  const stmt = db.prepare(`
    ...
    WHERE id = ?
  `)
  stmt.run(d.property_id, d.doc_type, d.title, d.file_path, d.issued_date, d.notes, d.linked_type ?? '', d.linked_id ?? 0, d.id)
}

// Palauttaa tiettyyn tietueeseen linkitetyt asiakirjat (esim. tulisijan nuohoustodistukset).
export function getDocumentsFor(linkedType: DocumentLinkType, linkedId: number): Document[] {
  const db = initDb()
  return db.prepare("SELECT * FROM documents WHERE linked_type = ? AND linked_id = ? ORDER BY issued_date DESC")
...
export function getMeterReadings(propertyId?: number): MeterReading[] {
  ...
  const query = propertyId
    ? db.prepare("SELECT * FROM meter_readings WHERE property_id = ? ORDER BY meter_type ASC, reading_date ASC")
    : db.prepare("SELECT * FROM meter_readings ORDER BY property_id ASC, meter_type ASC, reading_date ASC")
...
export function updateMeterReading(m: MeterReading): void {
  ...
  const stmt = db.prepare(`
    ...
    WHERE id = ?
  `)
  stmt.run(m.property_id, m.meter_type, m.reading, m.reading_date, m.notes, m.id)
}
