...
import { getProperties, getTransactions, getBookings, getUtilities, getTasks, type Transaction } from './db.js'
...
export function annualReport(year: number): AnnualReport {
  ...
  const byProperty: PropertyYearSummary[] = props.map(p => {
    ...
    return { propertyId: p.id, name: p.name, income, expense, net: income - expense }
  })

  const catMap = new Map<string, { category: string; type: Transaction['type']; total: number }>()
  for (const t of txs) {
    const key = `${t.type}:${t.category}`
    const cur = catMap.get(key) ?? { category: t.category, type: t.type, total: 0 }
    cur.total += t.amount
    catMap.set(key, cur)
  }
...
export function rentalIncomeReport(year: number): RentalReport {
  const props = getProperties()
  const txs = getTransactions().filter(t => inYear(t.date, year) && t.type === 'income' && t.category === 'Vuokraus')
  const bookings = getBookings().filter(b => inYear(b.start_date, year) && b.status !== 'cancelled')

  const rows: RentalReportRow[] = props.map(p => {
    const rentalIncome = txs.filter(t => t.property_id === p.id).reduce((s, t) => s + t.amount, 0)
    ...
    return { propertyId: p.id, name: p.name, kiinteistotunnus: p.kiinteistotunnus, rentalIncome, nights }
...
export function buildCsvExports(): Record<string, string> {
  return {
    'transactions.csv': toCSV(getTransactions() as unknown as Record<string, unknown>[]),
    'utilities.csv': toCSV(getUtilities() as unknown as Record<string, unknown>[]),
    'bookings.csv': toCSV(getBookings() as unknown as Record<string, unknown>[]),
    'tasks.csv': toCSV(getTasks() as unknown as Record<string, unknown>[]),
    'properties.csv': toCSV(getProperties() as unknown as Record<string, unknown>[])
  }
}
...
export function formatAnnualReport(r: AnnualReport): string {
  ...
  for (const p of r.byProperty) {
    lines.push(`  ${p.name.padEnd(16)} tulot ${eur(p.income).padStart(12)} | menot ${eur(p.expense).padStart(12)} | netto ${eur(p.net).padStart(12)}`)
  }
  lines.push('')
  lines.push('Kategorioittain:')
  for (const c of r.byCategory) {
    const label = c.type === 'income' ? 'TULO ' : 'MENO '
    lines.push(`  ${label}${c.category.padEnd(16)} ${eur(c.total).padStart(12)}`)
  }
  lines.push('')
  lines.push(`YHTEENSÄ  tulot ${eur(r.totalIncome)} | menot ${eur(r.totalExpense)} | NETTOTULOS ${eur(r.net)}`)
  return lines.join('\n')
}
...
export function formatRentalReport(r: RentalReport): string {
  ...
  if (r.rows.length === 0) {
    ...
    for (const row of r.rows) {
      lines.push(`  ${row.name.padEnd(16)} (${row.kiinteistotunnus.padEnd(14)}) vuokratulot ${eur(row.rentalIncome).padStart(12)} | ${row.nights} yötä`)
    }
    lines.push('')
